...
void HelloCardboardApp::realization0(float mainAngle) {
    vAngle = 0;
    alpha = 65.0f; float k = (360 - beta) / 65.0f;
    float m = 70 + 5*sqrt(1+k*k)/(sqrt(2.0f)*k - sqrt(1+k*k));
    float n = 180 - 5*sqrt(2.0f)/(sqrt(2.0f)*k - sqrt(1+k*k));
    float r = 5*sqrt(2.0f)*sqrt(1+k*k)/(sqrt(2.0f)*k - sqrt(1+k*k));
    if (mainAngle >= 0) {
//        vAngle = 180 / alpha * mainAngle;
		if (mainAngle < alpha) {
			vAngle = beta * mainAngle / alpha + (180-beta)*(mainAngle*mainAngle) / (alpha*alpha);
		}
		else if (mainAngle >= alpha && mainAngle < 70.0f) {
			vAngle = sqrt(r*r-(mainAngle-m)*(mainAngle-m)) + n;
		}
		else {
			vAngle = mainAngle + 110.0f + (5*sqrt(1+k*k)-5*sqrt(2))/(sqrt(2)*k-sqrt(1+k*k));
		}
...
//        vAngle = 180 / alpha * mainAngle;
    ...
    rotateM(rotated_head_view_, vAngle-mainAngle, head_view_, 0, -1, 0);
}


//non-linear amp
void HelloCardboardApp::realizationA(float mainAngle) {
    vAngle = 0;alpha = 65.0f;
    if (mainAngle > 0) {
        vAngle = 90 / alpha * mainAngle + 90 / (alpha*alpha) * mainAngle * mainAngle;
    }
    else {
        vAngle = 90 / alpha * mainAngle - 90 / (alpha*alpha) * mainAngle * mainAngle;
    }
...
void HelloCardboardApp::realizationB(float mainAngle) {
    vAngle = 0;alpha = 65.0f;
    if (mainAngle > 0) {
        vAngle = 90 / alpha * (mainAngle-alpha) - 90 / (alpha*alpha) * (mainAngle-alpha) * (mainAngle-alpha) + 180;
    }
    else {
        vAngle = 90 / alpha * (mainAngle+alpha) + 90 / (alpha*alpha) * (-mainAngle-alpha) * (-mainAngle-alpha) - 180;
    }
...
std::vector<float> HelloCardboardApp::ReturnVector() {
    ...
    res.push_back(PLAN == 0 ? viewAngle:vAngle);
    res.push_back(tamp);
    ...
    for (int i = 0; i < 16; ++i) {
            ...
            res.push_back(projection_matrices_[1][i]);

    }

    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            res.push_back(modelview_projection_room_.m[i][j]);
        }
    }
    return res;
}
...
//    }
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (PLAN == 0 || PLAN == 1 || PLAN == 2 || PLAN == 4) angle[i] = (angle[i]-iniAngle[i]) * 180 / PI;
...
//  direction = lAngle - angle[1] > 0 ? 1 : -1;
  ...
  if (PLAN == 0) {realization0(mainAngle); }
...
void HelloCardboardApp::SetParameter(float b) {
	beta = b;
}
...
void HelloCardboardApp::DrawRoom() {
  ...
  glUniformMatrix4fv(obj_modelview_projection_param_, 1, GL_FALSE,
                     room_array.data());
    LOGD("Obj_modelview = %d", obj_modelview_projection_param_);
  room_tex_.Bind();
  room_.Draw();
